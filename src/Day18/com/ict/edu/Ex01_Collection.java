...
public class Ex01_Collection {
	...
	// 컬렉션: 객체들을 모아서 관리하는 인터페이스들을 칭함
	// API: <T> => 객체타입, <T> => 요소(Element) = 컬렉션 안에 존재하는 객체 하나를 뜻한다.
	// 	    Map형식: <K,V> => Key, Value
	...
	// 							Iterator 객체를 반환, 요소들을 하나씩 꺼내서 작업 할 때 사용(for문도 사용가능)
	//
	// remove(Object o): boolean => 인자로 들어온 객체를 삭제, 성공하면 true
	// size(): int => 컬레션안에 존재하는 요소의 수(for문을 사용할 수 있다.)
