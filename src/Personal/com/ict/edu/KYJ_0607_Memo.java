...
public class KYJ_0607_Memo extends JFrame {
	JTextArea jta;
	JScrollPane jsp;
	JMenuBar jmb;
	...
	public KYJ_0607_Memo() {
		...
		i_newFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(size == 0) {
					...
					int res = JOptionPane.showOptionDialog(getParent(), "변경 내용을 제목 없음에 저장 하시겠습니까?", "간단 메모장", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
					if(res == 0) {
						// 실제 저장하는 코드
					}else if(res == 1) {
						jta.setText("");
		...
		i_exitFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				System.exit(0);
				// 원래는 내용이 변경되면 저장할지를 물어보고 변경되지 않으면 그냥 종료
			}
		});
